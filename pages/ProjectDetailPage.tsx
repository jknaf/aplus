              ...
              {project.productsUsed.map(product => (
                <li key={product.name}>
                  <Link to={product.path} className="group flex items-center text-brand-muted hover:text-brand-orange font-semibold transition-colors">
                     <span className="mr-2 text-brand-orange group-hover:translate-x-1 transition-transform duration-200" aria-hidden="true">&rarr;</span>
                    {product.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-20">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            ...
            <a href={shareLinks.twitter} target="_blank" rel="noopener noreferrer" aria-label="Auf X (Twitter) teilen" className="text-brand-muted hover:text-brand-orange transition-colors" title="Auf X (Twitter) teilen">
               <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
